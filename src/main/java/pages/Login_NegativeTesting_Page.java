...
public class Login_NegativeTesting_Page extends Helper2 {
    ...
    // Test 6: Invalid credentials (invalid email - valid credentials with a space afterwards for email and password)
    public void invalid_email_password_blankSpace(UserInfo userInfo) {
        clickElement(this.getDriver(), home_subtitle(), 3000);
        clickAndSendKeys(this.getDriver(), email_field(), userInfo.validUsernameBlankSpace, 2000);
        clickAndSendKeys(this.getDriver(), password_field(), userInfo.validPasswordBlankSpace, 3000);
        clickElement(this.getDriver(), login_button(), 3000);
        popUp2(this.getDriver());
    }
}
